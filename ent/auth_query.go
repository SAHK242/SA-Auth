...
func (aq *AuthQuery) QueryEmployee() *EmployeeQuery {
...
func (aq *AuthQuery) WithEmployee(opts ...func(*EmployeeQuery)) *AuthQuery {
	query := (&EmployeeClient{config: aq.config}).Query()
...
func (aq *AuthQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Auth, error) {
	...
	if query := aq.withEmployee; query != nil {
		if err := aq.loadEmployee(ctx, query, nodes, nil,
			func(n *Auth, e *Employee) { n.Edges.Employee = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AuthQuery) loadEmployee(ctx context.Context, query *EmployeeQuery, nodes []*Auth, init func(*Auth), assign func(*Auth, *Employee)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(employee.FieldEmployeeID)
	}
	query.Where(predicate.Employee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(auth.EmployeeColumn), fks...))
	}))
