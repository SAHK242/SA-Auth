...
func (eq *EmployeeQuery) QueryAuth() *AuthQuery {
	query := (&AuthClient{config: eq.config}).Query()
...
func (eq *EmployeeQuery) QueryDoctor() *DoctorQuery {
	query := (&DoctorClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EmployeeQuery) QueryNurse() *NurseQuery {
	query := (&NurseClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EmployeeQuery) Clone() *EmployeeQuery {
	...
	return &EmployeeQuery{
		...
		withAuth:       eq.withAuth.Clone(),
		withDepartment: eq.withDepartment.Clone(),
		withDoctor:     eq.withDoctor.Clone(),
		withNurse:      eq.withNurse.Clone(),
...
func (eq *EmployeeQuery) WithAuth(opts ...func(*AuthQuery)) *EmployeeQuery {
	query := (&AuthClient{config: eq.config}).Query()
...
func (eq *EmployeeQuery) WithDoctor(opts ...func(*DoctorQuery)) *EmployeeQuery {
	query := (&DoctorClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EmployeeQuery) WithNurse(opts ...func(*NurseQuery)) *EmployeeQuery {
	query := (&NurseClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Employee, error) {
	...
	if query := eq.withAuth; query != nil {
		if err := eq.loadAuth(ctx, query, nodes, nil,
			func(n *Employee, e *Auth) { n.Edges.Auth = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := eq.withDoctor; query != nil {
		if err := eq.loadDoctor(ctx, query, nodes, nil,
			func(n *Employee, e *Doctor) { n.Edges.Doctor = e }); err != nil {
			return nil, err
		}
	}
	if query := eq.withNurse; query != nil {
		if err := eq.loadNurse(ctx, query, nodes, nil,
			func(n *Employee, e *Nurse) { n.Edges.Nurse = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EmployeeQuery) loadAuth(ctx context.Context, query *AuthQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Auth)) error {
...
func (eq *EmployeeQuery) loadDoctor(ctx context.Context, query *DoctorQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Doctor)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(doctor.FieldDoctorID)
	}
	query.Where(predicate.Doctor(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.DoctorColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (eq *EmployeeQuery) loadNurse(ctx context.Context, query *NurseQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Nurse)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Employee)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(nurse.FieldNurseID)
	}
	query.Where(predicate.Nurse(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.NurseColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "nurse_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (eq *EmployeeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withAuth != nil {
			_spec.Node.AddColumnOnce(employee.FieldEmployeeID)
		}
